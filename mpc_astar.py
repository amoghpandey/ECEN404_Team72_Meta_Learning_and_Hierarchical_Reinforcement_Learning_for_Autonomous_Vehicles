...
#Offtheshelf MPC + modified A* and path smoothing. 
...
#############A-star#############################
#Base A* Credit: Christian Careaga(christian.careaga7@gmail.com) <-not for "safe" modification, for base A* only
def heuristic(a, b):
...
TARGET_SPEED = 55.0 / 3.6  # [m/s] target speed
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, initial_state,grid,start,goal):
    """
    Simulation
...
def main():
    ...
    t, x, y, yaw, v, d, a = do_simulation(
        cx, cy, cyaw, ck, sp, dl, initial_state,grid,start,goal)
    if show_animation:  # pragma: no cover
        plt.close("all")
        plt.subplots()
...
def main2():
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.subplots()
        plt.plot(t, (v*3.6), "-r", label="speed")
        plt.grid(True)
        plt.xlabel("Time [s]")
        plt.ylabel("Speed [kmh]")
